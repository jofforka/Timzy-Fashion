(() => {
  ...
  async function saveProduct(event) {
    ...
    const originalText = saveButton?.textContent || "Save Product";
    ...
    try {
      ...
      if (!product.name || !product.category || !Number.isFinite(product.price) || product.price < 0) {
        throw new Error("Complete the required product name, category, and price.");
      }

      if (saveButton) {
        saveButton.disabled = true;
        saveButton.textContent = state.pendingFiles.length ? "Uploading image..." : "Saving...";
      }
      ...
      if (state.pendingFiles.length) {
        newUrls = await uploadPendingImages(productId);
      }
      ...
      if (saveButton) saveButton.textContent = "Saving product...";

      await fb.db.collection(collections.products).doc(productId).set(product, { merge: true });

      toast(state.editingId ? "Product updated successfully." : "Product created successfully.");
      closeEditor();
      await loadProducts();
      renderAll();
    } catch (error) {
      ...
      let message = error?.message || "The product could not be saved.";

      if (error?.code === "storage/unauthorized") {
        message = "Image upload was blocked by Firebase Storage rules. Publish the included storage.rules file.";
      } else if (error?.code === "storage/bucket-not-found") {
        message = "Firebase Storage has not been created for this project. Open Firebase Storage and click Get started.";
      } else if (error?.code === "storage/unknown") {
        message = "Firebase Storage could not upload the image. Confirm Storage is enabled and the project billing plan supports it.";
      } else if (error?.code === "permission-denied") {
        message = "Firestore blocked the save. Publish the included firestore.rules file and remain signed in.";
      } else if (error?.code === "unauthenticated") {
        message = "Your login session has expired. Sign out and sign in again.";
      }
