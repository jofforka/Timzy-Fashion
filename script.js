...
function driveImages(value) {
  if (!value) return [];
  return String(value).split(",").map(link => driveToImage(link.trim())).filter(Boolean);
}
...
async function uploadImageFile(inputId) {
  ...
  $("uploadStatus") && ($("uploadStatus").textContent = `Uploading ${file.name}...`);
...
function setupRoleAccess(){
  ...
  if(currentRole === "admin"){
    showRoleSections(["dashboard","catalogManager","customerManager","sales","expenses","inventory","orders","publicOrders","customers","staff","forms"]);
  ...
  if(currentRole === "staff"){
    showRoleSections(["sales","expenses","inventory","orders","publicOrders","customers","forms"]);
    showStaffAdminFields(true);
    showTab("orders");
    return;
  }

  showRoleSections(["customers"]);
...
function hideAllSections(){
  ...
  document.querySelectorAll("#appMenu button").forEach(button => button.style.display = "none");
}
...
async function loadAllData(){
  ...
  inventory = inventoryData.map(row => {
    ...
    const uploaded = driveImages(
      n["product image"] || n["product images"] || n["image upload"] ||
      n["upload product image"] || n["product photo"] || n["product picture"] ||
      n["upload product picture"] || n["image"] || ""
    );
...
async function loadCatalog(){
  try{
    ...
    const firestoreCatalog = snap.docs.map(d => ({ id:d.id, source:"catalog", ...d.data() }));

    const inventoryCatalog = inventory.filter(x => x.product && x.product !== "-").map(x => ({
...
window.saveCatalogProduct = async function(){
  ...
  try{
    $("uploadStatus") && ($("uploadStatus").textContent = "Checking images...");

    const uploaded1 = await uploadImageFile("catFile1");
    const uploaded2 = await uploadImageFile("catFile2");
    const uploaded3 = await uploadImageFile("catFile3");

    if(uploaded1 && $("catImage1")) $("catImage1").value = uploaded1;
    if(uploaded2 && $("catImage2")) $("catImage2").value = uploaded2;
    if(uploaded3 && $("catImage3")) $("catImage3").value = uploaded3;
    ...
    $("uploadStatus") && ($("uploadStatus").textContent = "Saved successfully.");
    resetCatalogForm();
    await loadAllData();
    await loadFirestoreData();
    render();
    ...
    $("uploadStatus") && ($("uploadStatus").textContent = "Upload/save failed.");
  }
};

window.editCatalogProduct = function(id){
  const item = catalog.find(p => p.id === id);
  if(!item || item.source === "inventory") return alert("Inventory products must be edited from inventory/source.");
...
window.resetCatalogForm = function(){
  clearInputs([
    "editingProductId","catProductName","catCategory","catColor","catPrice","catQuantity","catSizes",
    "catBadge","catImage1","catImage2","catImage3","catDescription","catFile1","catFile2","catFile3"
  ]);
...
window.deleteCatalogProduct = async function(id){
  ...
  if(String(id).startsWith("inventory-")) return alert("This product came from Inventory. Remove it from Inventory.");
...
window.submitMeasurement = async function(){
  alert("Measurement portal is still available, but public catalog now collects measurements without login.");
};

window.submitOrder = async function(){
  alert("Customer portal order is still available. Public catalog buying is now handled on catalog.html.");
};

window.payOrder = async function(){
  alert("Use catalog.html checkout for public purchases.");
};
...
window.verifyPayment = async function(orderId){
  ...
  await updateDoc(doc(db,"customerOrders",orderId), {
    ...
    adminNote:note || "Payment confirmed.",
...
window.approveFulfillment = async id => updateOrderAdmin(id,"In Production","Order moved to production.");
window.markReady = async id => updateOrderAdmin(id,"Ready for Pickup/Delivery","Order is ready.");
window.markDelivered = async id => updateOrderAdmin(id,"Delivered","Order delivered.");
window.cancelOrder = async id => {
  const note = prompt("Cancellation reason:");
  if(!note) return;
  updateOrderAdmin(id,"Cancelled",note);
};
...
window.openOrderChat = function(){
  alert("Use WhatsApp for public catalog chat. Portal chat can be expanded later.");
};

window.sendChatMessage = function(){
  alert("Portal chat can be expanded later.");
};
...
function orderStatusCounts(){
  return {
    pending:orders.filter(order => String(order.status || "").toLowerCase().includes("pending")).length,
    paid:orders.filter(order => String(order.paymentStatus || "").toLowerCase() === "paid").length,
    delivered:orders.filter(order => String(order.status || "").toLowerCase().includes("delivered")).length
  };
}
...
function renderDashboardCharts(){
  ...
  drawBarChart("financeChart",["Sales","Expenses","Profit"],[stats.totalSalesAmount,stats.totalExpensesAmount,Math.max(stats.netProfitAmount,0)]);
  drawBarChart("orderStatusChart",["Pending","Paid","Delivered"],[counts.pending,counts.paid,counts.delivered]);
  drawBarChart("inventoryHealthChart",["Value","Low"],[stats.inventoryValueAmount,stats.lowStockCount]);
...
function statusClass(status=""){
  ...
  if(value.includes("paid") || value.includes("delivered") || value.includes("ready") || value.includes("production")) return "status-approved";
...
window.render = function(){
  ...
  if($("salesTable")) $("salesTable").innerHTML = sales.map(x => `<tr><td>${x.staff}</td><td>${x.category}</td><td>${x.product}</td><td>${x.qty}</td><td>${money(x.amount)}</td></tr>`).join("");
  if($("expensesTable")) $("expensesTable").innerHTML = expenses.map(x => `<tr><td>${x.staff}</td><td>${x.category}</td><td>${x.item}</td><td>${x.supplier}</td><td>${x.qty}</td><td>${money(x.unitCost)}</td><td>${money(x.total)}</td></tr>`).join("");
  if($("inventoryTable")) $("inventoryTable").innerHTML = inventory.map(x => `<tr><td>${x.category}</td><td>${x.product}</td><td>${x.sku}</td><td>${x.supplier}</td><td>${x.quantity}</td><td>${money(x.cost)}</td><td>${money(x.selling)}</td></tr>`).join("");
  ...
  const orderRows = orders.length ? orders.map(order => `
    <tr>
      ...
      <td class="table-actions">
        <button onclick="verifyPayment('${order.id}')">Verify</button>
        <button onclick="rejectPayment('${order.id}')">Reject Pay</button>
        <button onclick="approveFulfillment('${order.id}')">Production</button>
        <button onclick="markReady('${order.id}')">Ready</button>
        <button onclick="markDelivered('${order.id}')">Delivered</button>
        <button class="danger-btn" onclick="cancelOrder('${order.id}')">Cancel</button>
      </td>
    </tr>
  ...
  if($("customersTable")) $("customersTable").innerHTML = customers.length ? customers.map(customer => `
    <tr>
      <td>${customer.name || "-"}</td>
  ...
  if($("leaderboard")) $("leaderboard").innerHTML = staffXP().map((item,index) => `<div class="list-item"><b>#${index+1} ${item.name}</b><span>${item.points} XP</span></div>`).join("") || "<p>No XP yet.</p>";

  if($("formLinks")) $("formLinks").innerHTML = (window.TIMZY_FORMS || []).map(form => `<div class="glass-card"><h3>${form.name}</h3><p>${form.description}</p><a class="btn" href="${form.url}" target="_blank">Open</a></div>`).join("");
  ...
  if($("orderInsight")) $("orderInsight").textContent = "Portal order status overview.";
  ...
  grid.innerHTML = catalog.length ? catalog.map(item => {
    ...
    return `
      <div class="manager-card">
        <div class="manager-img">${img ? `<img src="${img}" alt="${item.name}">` : `<span>👗</span>`}</div>
        <div class="manager-body">
          <b>${item.name || "-"}</b>
          <small>${item.category || "-"} • ${item.productType || "Ready-Made"}</small>
          <strong>${money(item.price || 0)}</strong>
          <div class="table-actions">
            ${item.source === "inventory" ? `<button disabled>Inventory Item</button>` : `<button onclick="editCatalogProduct('${item.id}')">Edit</button><button class="danger-btn" onclick="deleteCatalogProduct('${item.id}')">Delete</button>`}
          </div>
        </div>
      </div>
    `;
